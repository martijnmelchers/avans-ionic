...
export class RoomService {
    ...
    GetRoom(): Observable<Room>{
        return new Observable<Room>((observer) => {
            this.socketService.on('joinedRoom',  (data: Room) => {
                observer.next(data);
            });
        });
    }
    ...
    JoinRoom(room: string){
        ...
        this.socketService.GetSocket().emit('joinRoom', {room: room, user: localStorage.getItem('jwt')});
        localStorage.setItem('roomId', room);
    }

    CreateRoom(room: string){
        // tslint:disable-next-line: object-literal-shorthand
        this.socketService.GetSocket().emit( 'createRoom', {room: room, user: localStorage.getItem('jwt')});
        localStorage.setItem('roomId', room);
    }
